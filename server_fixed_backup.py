...
class CustomerListHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_process_folder_files(self):
        ...
        with open('debug.log', 'a') as f:
            f.write("=== handle_process_folder_files START ===\n")
        try:
            import io
            import cgi
            ...
            with open('debug.log', 'a') as f:
                f.write(f"Read {len(data)} bytes of data\n")
            
            # Simple parsing to extract files
            all_results = []
            parts = data.split(b'--' + boundary)
            with open('debug.log', 'a') as f:
                f.write(f"Found {len(parts)} parts in multipart data\n")
            
            for i, part in enumerate(parts):
                with open('debug.log', 'a') as f:
                    f.write(f"Processing part {i}, length: {len(part)}\n")
                    if len(part) < 200:
                        f.write(f"Part content preview: {part}\n")
                    else:
                        f.write(f"Part content preview (first 100): {part[:100]}\n")
                
                if b'Content-Disposition: form-data' in part and b'filename=' in part:
                    with open('debug.log', 'a') as f:
                        f.write(f"Found form-data with filename in part {i}\n")
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if b'filename=' in clean_line:
                            ...
                            with open('debug.log', 'a') as f:
                                f.write(f"No filename in line: {clean_line}\n")
                    ...
                    if filename and filename.endswith('.xml'):
                        with open('debug.log', 'a') as f:
                            f.write(f"Processing XML file: {filename}\n")
